...
CREATE TABLE IF NOT EXISTS exophones (
    ...
    skip_call_logs       TINYINT NOT NULL DEFAULT 0 COMMENT '1 = skip writing call_logs during monitoring jobs',
...
CREATE TABLE IF NOT EXISTS heartbeat_metrics (
    ...
    transaction_id     VARCHAR(64) NOT NULL,
    account_id         BIGINT NOT NULL,
    exophone_id        BIGINT NOT NULL DEFAULT 0,
    ...
    INDEX idx_hm_exophone (exophone_id),
...
CREATE TABLE IF NOT EXISTS call_metrics_snapshot (
    ...
    leg1_no_answer       INT NOT NULL DEFAULT 0 COMMENT 'Leg1 calls where agent did not answer',
    leg1_busy            INT NOT NULL DEFAULT 0 COMMENT 'Leg1 calls where agent line was busy',
    leg1_failed          INT NOT NULL DEFAULT 0 COMMENT 'Leg1 calls that failed (network/system)',
    leg2_no_answer       INT NOT NULL DEFAULT 0 COMMENT 'Leg2 calls where customer did not answer',
    leg2_busy            INT NOT NULL DEFAULT 0 COMMENT 'Leg2 calls where customer line was busy',
    leg2_failed          INT NOT NULL DEFAULT 0 COMMENT 'Leg2 calls that failed (network/system)',
    leg2_canceled        INT NOT NULL DEFAULT 0 COMMENT 'Leg2 calls canceled before connect',
    created_at           TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at           TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
